  ...
  <div>
    ...
    <div class="navigator-search row justify-content-center">
      <div class="input-group col-lg-6">
        <input v-model="query" type="text" class="form-control" :placeholder="$t('navigator:search-input-placeholder')">
        <div class="input-group-append">

          <button @click="submitQuery()" class="btn btn-outline-secondary" :disabled="!query"
                  type="button">{{ 'navigator:search-button' | i18n }}
          </button>

          <button @click="reset()" class="btn btn-outline-secondary" :disabled="!query"
                  type="button">{{ 'navigator:clear-button' | i18n }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="navigator-path row">
      <div class="col-11 input-group">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item" v-for="package_ in path">
            <a v-if="isLast(path, package_)">{{package_.label}}</a>
            <router-link v-else :to="package_.id">{{package_.label}}</router-link>
          </li>
          <li v-show="query" class="breadcrumb-item">
            <span>{{ 'navigator:search-query-label' | i18n }}: <b>{{query}}</b></span>
          </li>
        </ol>
      </div>
      <div class="col-1">
    ...
    <b-modal v-if="nrSelectedItems > 0" id="deleteModal" ok-variant="danger" cancel-variant="secondary"
      ...
      {{ 'navigator:delete-confirmation-text' | i18n }}
    </b-modal>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      isSelected: function (item) {
        const itemIds = item.type === 'entity' ? this.selectedEntityTypeIds : this.selectedPackageIds
